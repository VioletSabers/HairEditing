...
class Blend:
    def __init__(self, image_face: torch.Tensor, image_hair: torch.Tensor, mask_face: torch.Tensor, mask_hair: torch.Tensor, *image_name):
        ...
        self.perceptual = VGG16_perceptual(requires_grad=False).cuda()
    ...
    def initlayer32(self, img):
        syn0, syn1, syn2, syn3 = self.perceptual(img) 
        return syn3
    ...
    def __call__(self, latent_in_m=None, noises_m=None):
        ...
        layer32 = self.initlayer32(self.image_face_d)
        layer32 = layer32.data
        layer32.requires_grad = True
        opt = optim.Adam([layer32] + noises, lr=cfg.blend.lr, betas=(0.9, 0.999), eps=1e-8)
        ...
        for e in range(cfg.blend.w_epochs):
            ...
            syn_img = self.G.module.mid_start(layer32, latent = latent_in, noise=noises, skip=None)
