...
class Reconstruction:
    ...
    def rec(self, image, image_name, mask_face=None, mask_hair=None, is_FS=True, front=False):
        print(f"\r\nStart Reconstruction -- {image_name} ")

        latent_in, noises = self.REC_S1.reconstruction(image, image_name, mask_face, mask_hair, front=front)
        print('\rStage2: ', image_name)
        F_init, skip = self.REC_S1.get_layerout(size=64)
        ...
        if is_FS:
            ...
            for epoch in range(cfg.rec.fs_epochs):
                ...
                syn_img = G.mid_start(F, S, noises, skip.detach(), size=64)
