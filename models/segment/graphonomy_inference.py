...
# Custom includes
from models.segment import graph

from models.segment import graphonomy_process as tr
...
def inference(net, img=None, use_gpu=True):
    ...
    adj2_test = (
        adj2_.unsqueeze(0).unsqueeze(0).expand(batch, 1, 7, 20).cuda().transpose(2, 3)
    )

    adj1_ = Variable(torch.from_numpy(graph.preprocess_adj(graph.pascal_graph)).float())
    adj3_test = adj1_.unsqueeze(0).unsqueeze(0).expand(batch, 1, 7, 7).cuda()

    cihp_adj = graph.preprocess_adj(graph.cihp_graph)
    adj3_ = Variable(torch.from_numpy(cihp_adj).float())
    adj1_test = adj3_.unsqueeze(0).unsqueeze(0).expand(batch, 1, 20, 20).cuda()
...
def get_3class(predictions):
    ...
    hair_pre = pre_sfm[:,2,:].unsqueeze(1)
    face_pre = pre_sfm[:,13,:].unsqueeze(1)
    bg_pre = torch.ones_like(hair_pre) - hair_pre - face_pre
    return torch.cat([bg_pre, face_pre, hair_pre], dim=1)

def get_faceclass(predictions):
    pre_sfm = F.softmax(predictions, dim=1)
    face_pre = pre_sfm[:,13,:]
    return face_pre

def get_hairclass(predictions):
    pre_sfm = F.softmax(predictions, dim=1)
    face_pre = pre_sfm[:,2,:]
    return face_pre

def get_target_mask(HM, FM):
    HM = HM.unsqueeze(0)
    FM = FM.unsqueeze(0)
    return torch.cat([HM, torch.zeros_like(HM), FM], dim=1)
